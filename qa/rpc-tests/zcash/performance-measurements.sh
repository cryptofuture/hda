...
function zcash_rpc {
    ./src/hda-cli -rpcwait -rpcuser=user -rpcpassword=password -rpcport=5983 "$@"
}
...
function zcashd_start {
    rm -rf "$DATADIR"
    mkdir -p "$DATADIR"
    ./src/hdad -regtest -datadir="$DATADIR" -rpcuser=user -rpcpassword=password -rpcport=5983 -showmetrics=0 &
    ZCASHD_PID=$!
}
