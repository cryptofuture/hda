...
UniValue GetNetworkHashPS(int lookup, int height) {
    ...
    // If lookup nonpositive, then use blocks since last difficulty averaging window.
    if (lookup <= 0)
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "hda is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "hda is downloading blocks...");
