...
#if defined(NDEBUG)
# error "hda cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "hda Signed Message:\n";
...
bool CheckTransactionWithoutProofVerification(const CTransaction& tx, CValidationState &state)
{
    ...
    // Transactions can contain empty `vin` and `vout` so long as `vjoinsplit` is non-empty.
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (!ContextualCheckInputs(tx, state, view, true, scriptVerifyFlags, true, Params().GetConsensus()))
...
namespace Consensus {
bool NonContextualCheckInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
{
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!ContextualCheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, txdata[i], chainparams.GetConsensus(), nScriptCheckThreads ? &vChecks : NULL))
